<?php
$result = mysql_query("SELECT * from admission where patients_info_id ='$patient_info_id' order by date_admitted DESC");
   $x=1;
    while($row = mysql_fetch_array($result))
    {
    //Explode Consultation Date
    $consultation_date = explode('-',$row['date_admitted']);
    $date_of_consultation = $consultation_date[1].'/'.$consultation_date[2].'/'.$consultation_date[0];
    $consult_date[$x] = $date_of_consultation;
    $id[$x] = $row['id'];
    $add[$x] = "<a href='admission_add_new_diagnosis.php?patients_info_id=$patient_info_id&admission_id=$row[id]'>ADD DIAGNOSIS</a> - <a href='admission_view_details.php?patients_info_id=$patient_info_id&admission_id=$row[id]'>VIEW/UPDATE</a>";
    $x++;
    }
...
<td align='center'><b>DIAGNOSIS STATUS</b></td>
    ...
    for($i = 1; $i < $x+1; $i++){
    $result1 = mysql_query("SELECT * from admission_details where admission_id ='$id[$i]'");
