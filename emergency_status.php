<?php
$result = mysql_query("SELECT * from emergency where patients_info_id ='$patient_info_id' order by date_referred DESC");
   $x=1;
    ...
    $id[$x] = $row['id'];
    $add[$x] = "<a href='emergency_add_new_diagnosis.php?patients_info_id=$patient_info_id&emergency_id=$row[id]'>ADD DIAGNOSIS</a> - <a href='emergency_view_details.php?patients_info_id=$patient_info_id&emergency_id=$row[id]'>VIEW/UPDATE</a>";
    $x++;
    }
...
<td align='center'><b>DIAGNOSIS STATUS</b></td>
    ...
    for($i = 1; $i < $x+1; $i++){
    $result1 = mysql_query("SELECT * from emergency_details where emergency_id ='$id[$i]'");
